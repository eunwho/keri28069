...
void MotorControlProc( )
{
    ...
    case 9: slip_comp_scalar_ctrl();        break;
    }
}
...
//    PERIOD_PWM_IRQ_SET;     // channel 1
    ...
    if( codeProtectOff < 0.5 ) {
        ...
        if((gPWMTripCode == 0 ) && ( gMachineState != STATE_POWER_ON)) gPWMTripCode = CheckOverCurrent();
    ...
    EPwm4Regs.CMPA.half.CMPA = MAX_PWM_CNT ;    // jsk debug 2021.02.14
    ...
    case STATE_WAIT_BREAK_OFF:
        if(gPWMTripCode !=0){
            ...
            if( codeMotorCtrlMode > 8.0 ){
                EPwm1Regs.CMPA.half.CMPA = MAX_PWM_CNT>>1;    // for keriInv
                EPwm2Regs.CMPA.half.CMPA = MAX_PWM_CNT>>1;
                EPwm3Regs.CMPA.half.CMPA = MAX_PWM_CNT>>1;
            }else{
